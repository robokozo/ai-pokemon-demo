...
export type AIStatus = typeof AI_UNAVAILABLE | typeof AI_LOADING | typeof AI_READY | typeof AI_ERROR
...
- Plain text only: no emojis, no emoticons, no asterisks, no underscores, no markdown of any kind.`
  ...
  function sanitizeResponse(text: string): string {
    ...
    const noEmoji = text.replace(/[\u{1F000}-\u{1FFFF}\u{2600}-\u{27BF}\u{2300}-\u{23FF}\u{2B00}-\u{2BFF}\u{1F900}-\u{1F9FF}\u{FE00}-\u{FEFF}]/gu, "")
  ...
  function getFallbackResponse(name: string): string {
    ...
    const pool = responses[name] ?? ["Hello there!", "Nice weather we are having.", "Have you visited the town inn lately?"]
